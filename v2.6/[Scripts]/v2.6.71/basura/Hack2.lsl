...
list Boxes = [
<224,2, 80.0>,
<53.86840, 158.02270, 80.0>,
<53.86840, 158.02270, 30.0>,

<146.16490, 158.90610, 80.0>,
<146.16490, 158.90610, 30.0>,

<187.84520, 68.16219, 80.0>,
<187.84520, 68.16219, 46.0>,

<166.93430, 219.29380, 80.0>,
<166.93430, 219.29380, 30.0>,

<187.88240, 174.86230, 80.0>,
<187.88240, 174.86230, 30.0>,

<128.96990, 76.33781, 80.0>,
...
<107.00680, 134.10600, 80.0>,
<107.00680, 134.10600, 30.0>,

<105.84550, 196.01010, 80.0>,
<105.84550, 196.01010, 30.0>,

<95.91953, 69.16455, 80.0>,
<95.91953, 69.16455, 30.0>,

<0,0,0>
];
float XDXD = 3.0;//reach to same position
float asdaswd = 80.0;//axis Z
float asd = 30.0;
float SPEED_LINDEN_REALMS5 = 0.044444444;//speed
...
ProLAVA(list pos,float SPEED2X,float dists)
{
    ...
    else if(OKEYLOL==0)
    {
        ...
        if(ok<=dists)
        {
            ...
            //llSleep(0.1);
            //llOwnerSay("SENSOR");
            //llSensor("","",(AGENT|SCRIPTED),95.0,PI,0.1);
            A+=1;
            OKEYLOL=1;
            YALOMANDASTE=0;
        }
    }
    if(OKEYLOL==1 & YALOMANDASTE==0)
    {
...
default
{
       ...
       // {
           ...
           llOwnerSay("SI");
            ...
            while(detected--)
            {
                ...
                string name = llKey2Name(keys);
                if(name=="Amulet")
                {
                    GRZ = llGetObjectDetails(keys,[OBJECT_POS]);
            ...
            if(serverside==1)
            {
                while(detected2--)
                {
                    ...
                    if(llGetAgentSize(keys)) 
                    {
                        ...
                        float ok = (float)llVecDist((vector)llList2Vector(PERSONA,0),(vector)llList2Vector(GRZ,0));
                        if(ok>5.0)
                        {
            ...
            while(detected--)
            {
                ...
                if(name=="LR - Floor"|name=="LR - Collision"|name=="LR - Grass")
                {
                    ...
                    if(<ZXA.x,ZXA.y,0>!=<1.00000,1.00000,0> & llList2String(DIAMANTE,1)=="81920" & llGetOwnerKey(keys)!=NULL_KEY & ZXA!=ZERO_VECTOR)
                    {
                        ...
                        vector OKEZ = llGetPos();
                        vector DOWN = ZXA;
                        ZXA=<ZXA.x,ZXA.y,OKEZ.z+OFFSET>;
                        integer RAKE;
                        integer tis;
                        @returns;
                        tis+=1;
                        if(llGetOwnerKey(keys)!=NULL_KEY & tis<LOL)
                        {
                            if(RAKE==0)
                            {
                                MoveTarget2(ZXA,owner,SPEED2);
    ...
    link_message(integer sender_num, integer num, string str, key id)
    {
        ...
        {
            FXDUKS=0;
            //llSensorRepeat("","",(AGENT|SCRIPTED),20.0,PI,0.1);
        }
        else if(str=="XSTOPX")
        {
            RUNING=0;
        ...
        else if(llGetSubString(str,0,1)=="XC")
        {
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        MoveTarget2(finalmeta,owner,SPEED2);
                    }
                    if(llVecDist(llGetPos(),finalmeta)<=1.6)//al tras cristales usa la distancia esta
                    {
                        ...
                        {
                            //llSensor("","",(AGENT|SCRIPTED),0x7FFFFFFF,PI);
                        }
                    }
                }
            }
