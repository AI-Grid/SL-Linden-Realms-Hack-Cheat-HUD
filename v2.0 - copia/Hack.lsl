...
// cuando termino de teletransportarme luego de las rocas de lava en LR167 no me va al paid pad ??
integer MERGES;
...
list LR4 = ["LR 2","LR 22","LR 42","LR 142","LR 162","LR 182"]; // REGION LR 2 NO FUNCIONA
...
//list POSRegion5 = [<222,3,44>,<222,3,44>,<222,3,34>,<223,0,44>];
vector POSRegion5 = <222,3,28>;
//vector POSRegion5 = <226.40240, 233.82050, 46>;

list POSRegion6 = [<58,0,48>,<58,0,48>,<58,-1.54533,48>];

list POSRegion7 = [<0,214,37>,<0,214,37>,<-3.8,214,37>];

list POSRegion8 = [<241,130,63>,
<58,0,63>,
...
list POSRegion11 =  [<204.65760, 17.37965, 1002.26801>,
<204.65760, 17.37965, 1002.26801>,
<206.33770, 41.05254, 1040.11812>,
<202.07170, 69.61992, 1040.40075>,
<198.78800, 123.50960, 1040.11446>,
<191.76520, 159.68590, 1040.96450>,
<188.06380, 192.69030, 1040.00345>,
<183.82900, 230.86080, 1040.60505>,
<182.27390, 248.26350, 1040.67903>,
<182.27390, 248.26350, 1040.67903>,
<182.27390, 260.26350, 1040.67903>];
...
list POSRegion12C = [<36,112,41>,<36,112,41>,<36,105,42>];

vector POSRegion12F = <226.40240, 233.82050, 46>;//= <229.68820, 234.88120, 44.98196>;
...
AirGO()
{
    ...
    else if(~llListFindList(LR1,(list)region))
    {
        if(MERGES%2)
        {
            ...
            if((float)llVecDist((vector)llGetPos(),(vector)POSRegion12F)<1.09)
            {
                llStopMoveToTarget();
                MERGES=0;
                llOwnerSay(xD+" Obteniendo los lindens...");
                llOwnerSay(xD+" Apagando script...");
                llOwnerSay(xD+" Puede usar el hud en otro alter y seguir.");
                llOwnerSay(xD+" FIN DEL JUEGO.");
                llSetText("FIN DEL JUEGO.",color,1.0);
    ...
    else if(~llListFindList(LR10,(list)region))
    {
        ...
        {
            XKS=1;
            PROFinder(POSRegion12,SPEED_LINDEN_REALMS45,2.0);
            if(finishs==<148.13300, 162.64560, 80.40981> & CA==0)
            {
                CA=15;
                llStopMoveToTarget();
                llSetText("POR FAVOR CLICKEA LA BOTELLA DE LA MESA Y ENTRA EN LA PLATAFORMA! \n Cuando estes DENTRO DE LA LAVA DE ROCAS clickea el HUD Flecha.",color,1.0);
                llTargetOmega(<0.1,0.0,0.0>,TWO_PI,0.1);
                llOwnerSay(xD+" POR FAVOR CLICKEA LA BOTELLA DE LA MESA Y ENTRA EN LA PLATAFORMA! Cuando estes DENTRO DE LA LAVA DE ROCAS clickea el HUD Flecha.");
            }
        }
    }
...
default
{
    ...
    sensor(integer detected)
    {
        while(detected--)
        {
            ...
            name=="Air Blob"|
            name=="Transporter"|
            name=="Very Giant Mud Ball - Swamp"|
            name=="Giant Lava Ball - Mountain"|
            name=="Very Giant Lava Ball - Mountain"|
            name=="Fallen Rock"|
            name=="Fallen Rock 0"|
            name=="Fallen Rock 1"|
            name=="Fallen Rock 2"|
            ...
            {
                llOwnerSay(xD + " (SOLAMENTE SI SE TRABA) DEBE ALEJARSE DE ESTA PLATAFORMA Y REINICIAR SCRIPT.");
                llResetScript();
            }
        }
    }
