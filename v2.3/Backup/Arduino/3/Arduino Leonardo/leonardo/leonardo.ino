...
unsigned int DondeEmpezarVARIABLE = 0; // Numero de cuenta por empezar, cuentas guardadas en el script del arduino MEGA.
unsigned int CuentaDondeFinalizar = 79; // Numero de cuenta que finalizara.
...
void loop() 
{
    ...
    if(OK2%2)
    {
        if(CuentaDondeEmpezar==DondeEmpezarVARIABLE|analogRead(Pin_Microfono)==1023)
        {
            ...
            if(clave==""|clave==" ")
            {
                ...
                goto agaX;
            }//http://maps.secondlife.com/secondlife/Wengen/18/226/86
            prowrite("C:/Firestorm-releasex64/Firestorm-releasex64.exe --noprobe --set InstallLanguage es --no-verify-ssl-cert --login "
            +RequestInformation(CuentaDondeEmpezar)
            +" Resident ERROR Wengen/18/226/86"
            +" & exit" 
            ,true);
            delay(500);
            Keyboard.releaseAll();
