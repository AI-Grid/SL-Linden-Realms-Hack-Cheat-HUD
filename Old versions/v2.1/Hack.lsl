...
float SPEED_LINDEN_REALMS5X = 3.5;
...
string nasB = " Debe obtener (10 ROJOS 5 AMARILLOS 3 NARANJAS 1 VERDE). Puede utilizar 'CTRL + SHIFT + R'. Cuando este listo clickee el boton 'LISTO -->>'.";
        ...
        if(RCDI==0)
        {
            ...
            else if(~llListFindList(LR10,(list)region))
            {
                ...
                else if(CA==16)
                {
                    ...
                    {
                        CA=17;
                        llSetText("Por favor clickee el amuleto en el suelo que esta cerca ti.\n Cuando estes listo clickee el boton 'LISTO -->>'.",color,1.0);
                        llTargetOmega(<0.1,0.0,0.0>,TWO_PI,0.1);
                        llOwnerSay(xD+" Por favor clickee el amuleto en el suelo que esta cerca ti. Cuando estes listo clickee el boton 'LISTO -->>'.");
                        llStopMoveToTarget();
                    }
                }
                else if(CA==0)
            ...
            else if(~llListFindList(LR8,(list)region))
            {
                ...
                {
                    PROFinder(POSRegion9,SPEED_LINDEN_REALMS5X,2.0);
            ...
            else if(~llListFindList(LR4,(list)region))
            {
                ...
                else if(CA==0)
                {
                    ...
                    if((float)llVecDist((vector)llGetPos(),(vector)POSRegion5)<0.6)
                    {
                        ...
                        llStopMoveToTarget();
                        CA=500;
                        llSetText("Por favor clickee los lentes del suelo y apriete M.",color,1.0);
                        llTargetOmega(<0.1,0.0,0.0>,TWO_PI,0.1);
                        llOwnerSay(xD+" Por favor clickee los lentes del suelo y apriete M.");
                        llSend("CAMARASENSOR");
                    }
                }
                else if(CA==500)
                {
                    if(llGetPermissions() & PERMISSION_CONTROL_CAMERA)
                    {
                        if(llGetAgentInfo(llGetOwner()) & AGENT_MOUSELOOK)
                        {
                            ...
                            //CA=600;
                            CA=5;
                            llSetText("Debe ir corriendo al PUNTO ROJO en el RADAR de los LENTES. \n Luego de estar cerca del PUNTO ROJO vera una CAJA, clickee en la misma. \n Cuando estes listo clickee el boton 'LISTO -->>'.",color,1.0);
                            llTargetOmega(<0.1,0.0,0.0>,TWO_PI,0.1);
                            llOwnerSay(xD+" Debe ir corriendo al PUNTO ROJO en el RADAR de los LENTES. Luego de estar cerca del PUNTO ROJO vera una CAJA, clickee en la misma. Cuando estes listo clickee el boton 'LISTO -->>'.");
                        }
                    }
                }
                //else if(CA==600)
                //{//CA=5;
                    //SWITCH CAMBIADOR DE PARA AGARRAR
                //}
            }
                ...
                else if(ZK!=1)
                {
                    ...
                    else if(CA==2)
                    {
                        ...
                        if(finishs==<26,228,56>)
                        {
                            llStopMoveToTarget();
                            CA=3;
                            llSetText("Debera clickear este arbol tambien! \n Cuando estes listo clickee el boton 'LISTO -->>'.",color,1.0);
                            llTargetOmega(<0.1,0.0,0.0>,TWO_PI,0.1);
                            llOwnerSay(xD+" Debera clickear este arbol tambien! Cuando estes listo clickee el boton 'LISTO -->>'.");
                        }
                    }
                    else if(CA!=1)
                    {
                            ...
                            if(finishs==<20,242,26>)
                            {
                                ...
                                if(MSI%2)
                                {
                                    if(CA==0 & NO!=1)
                                    {
                                        ...
                                        llOwnerSay(xD+nasB);
                                        RCDI=1;
                                        llOwnerSay(xD + " AutoLindens esta activado, espere unos segundos.");
                                        llSleep(4.0);                           
